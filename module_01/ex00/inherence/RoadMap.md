Create a Tool abstract class and two derived classes, Hammer and Shovel

- [x] Worker class
    - [x] not use Shovel but has Tools
    - [x] multiple tools, be able to return them (give them back ??)
    - [x] vector of Tool * to represent all the tools
- [x] Create an abstract class Tool.
    - [x] protected int numberOfUses
    - [x] pure virtual method use()
- [x] Make the class Shovel inherit from Tool.
- [x] Create a class Hammer that inherits from Tool.
- [x] Ensure that tools can be given to a Worker and taken away.
- [x] Ensure that, if the tool is already given to a worker, giving it to another worker does remove it from the first